...
const UsersPage: React.FC = () => {
  ...
  const handleEditUser = (user: ApiUser) => {
    setEditingUser(user);
    setNewlyCreatedPassword(''); // Clear any newly created password display
    formik.setValues({
      id: user.id,
      ...
      {newlyCreatedPassword && (
        <Alert severity="info" sx={{ mb: 3 }} onClose={() => setNewlyCreatedPassword('')}>
          ...
          <Typography variant="body2">
            Username: <strong>{formik.values.username}</strong>
          </Typography>
          <Typography variant="body2">
            Password: <strong>{newlyCreatedPassword}</strong>
          </Typography>
          <Typography variant="caption" sx={{ mt: 1, display: 'block' }}>
            Please save this password. It will be encrypted in the database and won't be visible again.
          </Typography>
        </Alert>
      )}
