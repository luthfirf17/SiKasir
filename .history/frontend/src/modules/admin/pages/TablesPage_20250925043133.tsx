...
// PERBAIKAN 1: Hapus definisi Table lokal dan impor langsung dari sumbernya
import { api, apiConfig, Table, TableStats } from '../../../services/api'; 

// --- Definisi Tipe Data Tambahan ---
...
const StatusUpdateDialog: React.FC<StatusUpdateDialogProps> = ({ open, onClose, onUpdate, setNotification, table }) => {
  const [statusData, setStatusData] = useState<{ status: Table['status']; notes: string }>({ status: table.status, notes: table.notes || '' });
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <Grid container spacing={2} sx={{ mt: 1 }}>
          <Grid item xs={12}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                // PERBAIKAN 2: Menggunakan type assertion 'as' untuk mengatasi error Tipe String
                onChange={(e) => setStatusData(p => ({ ...p, status: e.target.value as Table['status'] }))}
              >
          ...
          <Grid item xs={12}>
            <TextField label="Catatan" value={statusData.notes} onChange={(e) => setStatusData(p => ({ ...p, notes: e.target.value }))} fullWidth multiline rows={3} />
          </Grid>
        </Grid>
      </DialogContent>
      <DialogActions>
...
const AreaManagementDialog: React.FC<AreaManagementDialogProps> = ({ open, onClose, setNotification, areaOptions, setAreaOptions, tables }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box mb={2}>
            <Typography variant="h6">Tambah Area Baru</Typography>
            <Grid container spacing={1} alignItems="center">
                <Grid item xs={5}><TextField label="Value" size="small" value={newArea.value} onChange={(e) => setNewArea(p => ({ ...p, value: e.target.value.toLowerCase().replace(/\s/g, '_') }))} fullWidth helperText="e.g., smoking_area" /></Grid>
                <Grid item xs={5}><TextField label="Label" size="small" value={newArea.label} onChange={(e) => setNewArea(p => ({ ...p, label: e.target.value }))} fullWidth helperText="e.g., Smoking Area" /></Grid>
                <Grid item xs={2}><Button variant="contained" onClick={handleAddArea} fullWidth sx={{ height: '40px' }}><AddIcon /></Button></Grid>
            </Grid>
        </Box>
