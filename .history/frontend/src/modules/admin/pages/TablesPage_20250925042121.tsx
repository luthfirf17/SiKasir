  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (tablesResponse.data && Array.isArray(tablesResponse.data)) {
        setTables(tablesResponse.data.filter(t => t && t.table_id));
      }
    ...
    <Box p={3}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} md={5}><TextField fullWidth size="small" label="Cari Meja" value={filters.query} onChange={(e) => setFilters(p => ({ ...p, query: e.target.value }))} /></Grid>
...
// Dialog untuk Tambah/Edit Meja
const TableFormDialog = ({ open, onClose, onSave, setNotification, table, areaOptions }: any) => {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Grid container spacing={2} sx={{ mt: 1 }}>
          ...
          <Grid item xs={12}><FormControl fullWidth><InputLabel>Area</InputLabel><Select name="area" label="Area" value={formData.area} onChange={handleChange as any}>{areaOptions.map((opt: AreaOption) => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}</Select></FormControl></Grid>
...
const StatusUpdateDialog = ({ open, onClose, onUpdate, setNotification, table }: any) => {
...
// Dialog untuk Manajemen Area
const AreaManagementDialog = ({ open, onClose, setNotification, areaOptions, setAreaOptions, tables }: any) => {
  ...
  const handleAddArea = () => {
    ...
    if (areaOptions.some((opt: AreaOption) => opt.value === newArea.value)) return setNotification({ message: 'Value area sudah ada.', type: 'error' });
  ...
  const handleDeleteArea = (value: string) => {
    if (tables.some((t: Table) => t.area === value)) return setNotification({ message: 'Tidak bisa menghapus, area masih digunakan oleh meja.', type: 'error' });
    setAreaOptions(areaOptions.filter((opt: AreaOption) => opt.value !== value));
      ...
      <DialogContent>
        ...
        {areaOptions.map((opt: AreaOption) => (
          <Paper key={opt.value} sx={{ p: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
            ...
            <IconButton size="small" color="error" onClick={() => handleDeleteArea(opt.value)}><DeleteIcon /></IconButton>
          </Paper>
        ))}
      </DialogContent>
