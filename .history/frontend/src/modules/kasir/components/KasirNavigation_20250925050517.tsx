...
import { Drawer, List, ListItem, ListItemIcon, ListItemText, Typography, Box, Divider, Avatar, Chip } from '@mui/material';
import { PointOfSale, Payment, Receipt, History, AccountBalance, TrendingUp, Schedule, ExitToApp, Notifications } from '@mui/icons-material';
...
const KasirNavigation: React.FC<KasirNavigationProps> = ({
  ...
  return (
    ...
    >
      {/* ========== HEADER USER INFO ========== */}
      {/* Menampilkan avatar, nama, dan role kasir */}
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Avatar sx={{ width: 60, height: 60, bgcolor: 'success.main', margin: '0 auto', mb: 2 }}>
          {user?.fullName?.charAt(0) || 'K'}
        </Avatar>
        <Typography variant="h6" fontWeight="bold">
          {user?.fullName || 'Kasir'}
        </Typography>
        <Chip label="Cashier" color="success" size="small" sx={{ mt: 1 }} />
      </Box>

      <Divider sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />

      {/* ========== NAVIGATION MENU ========== */}
      {/* List menu navigasi utama untuk kasir */}
      <List sx={{ px: 2, py: 1 }}>
        {KASIR_MENU_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <ListItemIcon sx={{ color: isActive ? 'success.main' : 'white' }}>
                {item.icon}
              </ListItemIcon>
              <ListItemText
                primary={item.text}
                primaryTypographyProps={{
                  ...
                  color: isActive ? 'success.main' : 'white',
                }}
              />
            </ListItem>
          );
      ...
      {/* ========== FOOTER ACTIONS ========== */}
      {/* Menu tambahan untuk notifikasi dan logout */}
      <List sx={{ px: 2, py: 1 }}>
        <ListItem
        ...
        >
          ...
          <ListItemText primary="Notifications" primaryTypographyProps={{ color: 'white' }} />
